...
describe('Moxiequire Service Unit Tests:', function() {

	it('should get mock path', function(done) {
		const filePath = moxiequire.getMockPath('./util/server/services/util.server.service.js');
	...
	it('should be able to get an external mock', function(done) {
		const nodemailer = moxiequire.getExternalMock('nodemailer');
	...
	it('should be able to mock an external module from a separate controller', function(done) {
		const utilService = moxiequire.mockFile('./util/server/services/util.server.service.js');
	...
	it('should expose access to service for direct modification', function(done) {
		const utilService = moxiequire.mockFile('./util/server/services/util.server.service.js', {directModification: ()=>42});
	...
	it('should be able to instantiate a mock service from a file', function(done) {
		const utilService = moxiequire.mockFile('./util/server/services/util.server.service.js');
		should.exist(utilService.isNotMocked);
		done();
	});
