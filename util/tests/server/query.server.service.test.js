...
describe('Query Server Service Tests:', function() {
	...
	it('should format parameters', function(done) {
		...
		const param1 = queryService.formatQueryParam({ project: {value: '5702c24b41ef4dce67d2dfba', dataType: 'ObjectId'}});
		param1.project._bsontype.should.equal('ObjectID');
		param1.project.toString().should.equal('5702c24b41ef4dce67d2dfba');

		// Test Date conversion
		const date = new Date();
		const param2 = queryService.formatQueryParam({ created: { $gte: {value: date.toString(), dataType: 'date'} } });
		...
		const param3 = queryService.formatQueryParam({blah: 1});
		param3.blah.should.equal(1);

		// Test direct string value
		const param4 = queryService.formatQueryParam({blah: 'whee'});
		param4.blah.should.equal('whee');

		// Test nested query params with date, and objectID conversions
		const startDate = new Date(date.getDate()-5);
		const param5 = queryService.formatQueryParam({
			$and: [
		...
		// Test empty object
		const param6 = queryService.formatQueryParam({});
		(typeof param6).should.equal('object');
		Object.keys(param6).length.should.equal(0);

		// Test null object
		const param7 = queryService.formatQueryParam();
		(param7 === null).should.equal(true);

		// Null value
		const param8 = queryService.formatQueryParam({testing: null});
	...
	it('should delete dangerous mongo query keys', function(done) {
		...
		const param1 = queryService.formatQueryParam({ $where: 'stuff' });
		(param1 === null).should.equal(true);

		// Test blacklisted phrase 'mapReduce'
		const param2 = queryService.formatQueryParam({ mapReduce: 'stuff' });
		(param2 === null).should.equal(true);

		// Test blacklisted phrase 'group'
		const param3 = queryService.formatQueryParam({ group: 'stuff' });
		(param3 === null).should.equal(true);

		// Test nested query with dangerous keys
		const param4 = queryService.formatQueryParam({
			$or: [
				{$and: [
					{ test: 1 },
					{ $where: 'stuff'}
