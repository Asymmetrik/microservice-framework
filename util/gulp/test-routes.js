...
module.exports = function(silent) {
	...
	return through.obj(function(vinylFile, encoding, next) {
		mongoosePromise.then(function() {
			// load the file
			var util = require(path.resolve('./test-route.server.service.js'));
			var file = require(vinylFile.path);
			if (!file) {
				return next(null, vinylFile);
			}
			var fileName = vinylFile.path.substring(vinylFile.path.lastIndexOf('/') + 1);
			// load the test
			var testFile;
			try {
				var matches = /app\/(.+)\/server\/routes\/(.+).js/g.exec(vinylFile.path);
			...
			var mockRoutes = function(defer, routeNameIndex, routeNames, routeTypeIndex, routeTypes) {
				...
				var routeName = routeNames[routeNameIndex];
				var routeType = routeTypes[routeTypeIndex];
				...
				clearDB().then(function() {
					...
					_.each(testFile[routeName][routeType], function(test, index) {
						...
						var outputError = function(errors) {
							++totalNumberOfErrors;
							passed = 0;
							var name = _.isEmpty(test) ? 'default test' : test.name || 'test #' + index;
							var message = 'Failed ' + name + ' for ' + routeName + ' (' + routeType + ') in ' + fileName + ':';
						...
						testPromise = testPromise.then(function() {
							...
							util.testRoute(file, routeName, routeType, test.paramArgs || [], req, function(error, response) {
								...
								q.when({}).then(function() {
									if (test.validate) {
										var cleanRes = _.transform(response, function(result, value, key) {
							...
							return q.allSettled(_.map(mongoose.modelNames(), function(modelName) {
								var expectedCount = testCounts[modelName] || 0;
					...
					return testPromise;
				}).done(next); // move on to the next test
			};
		...
		mongooseObj.disconnect(function() {
			next(totalNumberOfErrors);
		});
	});
};