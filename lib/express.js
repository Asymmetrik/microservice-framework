...
function initMiddleware(app) {
	...
	// Use extended URL parsing by default, but this can be overridden in the config
...
function initSession(app, sessionOverride) {
	// If a session was provided, use that instead of the default
	if (sessionOverride) {
	...
	app.use(session({
		...
		store: mongoStore,
		saveUninitialized: config.auth.saveUnitialized !== undefined ? config.auth.saveUnitialized : true,
		resave: config.auth.resave !== undefined ? config.auth.resave : true,
